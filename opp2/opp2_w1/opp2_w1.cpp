...
void matrixAndVectorParallelMultiplication(int size, double* matrix, double* vector, double* result) {
    // parallel
...
void parallelScalarProduct(int size, double* vector1, double* vector2, double* result_parts) {
    // parallel
...
#pragma omp parallel
    {
        ...
        int num_threads = omp_get_num_threads();    // number of processes
        int rank = omp_get_thread_num();    // id of current process
        int elements_per_process = size / num_threads;
        int first_process_element = rank * elements_per_process;
        double sum = 0; // scalar product sum's part
...
double SoLESolutionParallelTest(int num_threads, int size, double epsilon, int test_type) {
    ...
    // parts of scalar product
    double* sp_parts = (double*) malloc(num_threads * sizeof(double));
    ...
    while (true) {
        // STEP 1/3: y^n = Ax^n - b
        // mult = A * x
        matrixAndVectorParallelMultiplication(size, A, x, mult);
        // y = mult - b
        ...
        // mult = A * y
        matrixAndVectorParallelMultiplication(size, A, y, mult);

        // counting first scalar product
        double sp1 = 0;
        parallelScalarProduct(size, y, mult, sp_parts);
        // uniting parts of scalar product together
        for (int i = 0; i < num_threads; i++) {
            sp1 += sp_parts[i];
        }

        // counting second scalar product
        double sp2 = 0;
        parallelScalarProduct(size, mult, mult, sp_parts);
        // uniting parts of scalar product together
        for (int i = 0; i < num_threads; i++) {
            sp2 += sp_parts[i];
        }
